...
let reservations = [];
try {
  const raw = localStorage.getItem("calendar-reservations");
...
function openReservationModal(dateStr, cell) {
      ...
      <div class="resv-modal">
        ...
        <form id="resv-inline-form">
          <div style="margin-top:12px;display:flex;gap:8px;justify-content:flex-end;">
            ...
            <button class="btn-primary checkBtn">Confirmer</button>
          </div>
        </form>
      </div>
    `;
  ...
  /*try {
    ...
    if (reservations[dateStr]) {
      document.getElementById("r-name").value =
        reservations[dateStr].name || "";
      document.getElementById("r-email").value =
        reservations[dateStr].email || "";
    }
  } catch (e) {
    console.error(e);
  }*/
  ...
  checkBtn.addEventListener("click", (e) => {
    ...
    //localStorage.setItem("calendar-reservations", "pouet");
